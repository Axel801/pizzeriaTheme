...
add_action('plugins_loaded', 'altp_pizzeriadb_revisar');
function altp_pizzeriadb_revisar(){
  global $altp_dbversion;
...
add_action( 'switch_theme', 'altp_remove_database' );
function altp_remove_database(){
  // WPDB nos da los métodos para trabajar con tablas
  global $wpdb;
  $sql = "DROP TABLE ".$wpdb->prefix."reservations";

  $wpdb->query($sql);
  delete_option('altp_pizzeria_dbversion');
  delete_option('altp_pizzeria_address');
  delete_option('altp_pizzeria_gmap_apikey');
  delete_option('altp_pizzeria_gmap_latitude');
  delete_option('altp_pizzeria_gmap_longitude');
  delete_option('altp_pizzeria_gmap_zoom');
  delete_option('altp_pizzeria_public_key');
  delete_option('altp_pizzeria_secret_key');
  delete_option('altp_pizzeria_tel');
}
